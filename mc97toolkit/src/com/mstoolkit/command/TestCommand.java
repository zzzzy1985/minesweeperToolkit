...
public class TestCommand implements ICommand
{
    public void execute(JFrame frame)
    {
        ...
        String strid = JOptionPane.showInputDialog("参数，依次为宽,长,雷,样本数(长,宽1-2位数字,雷2位数字,样本最大为1亿)", "8,8,10,1000");
        if (strid == null)
        {
            return;
        }
        if (strid.equals(""))
        {
            JOptionPane.showMessageDialog(frame, "请输入参数", "提示信息", JOptionPane.ERROR_MESSAGE);
            return;
        }
        else if (strid.matches("^[1-9]([0-9]{0,1}),[1-9]([0-9]{0,1}),[1-9]([0-9]{1,1}),[1-9]([0-9]{0,10})$"))
        {
        ...
        {
            JOptionPane.showMessageDialog(frame, "格式不正确", "提示信息", JOptionPane.ERROR_MESSAGE);
            return;
        }
    }
